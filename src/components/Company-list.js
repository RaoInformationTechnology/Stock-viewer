...
displayCompanyList(){
	...
	if(this.state.isOpenSearch && !this.state.isSearchClick){
		...
		return(
			...
			variant="outlined"
			/>
			<Button id="search"  onClick={this.handleSubmit} style={{color:'#fff'}} autoFocus>
			Search
			</Button>
			</Typography>
			</div>					
		...
		if(!this.state.searchResponse.length){
			...
			return(
				...
				variant="outlined"
				/>
				<Button className="search_button" id="search" onClick={this.handleSubmit} style={{color:'#fff'}} autoFocus>
				Search
				</Button>
				</Typography>
