...
class Companylist extends Component {
	...
	displayCompanyList(){
		...
		var dispayCompany = this.state.companyData.length ? <div>{this.state.companyData.map(company =>
			...
			)} </div> : <div> <p>Add Comapany to watchlist</p></div>
		...
		if(this.state.isOpenSearch && !this.state.isSearchClick){
			return(
				...
				<Typography variant="h6" noWrap>
					...
					variant="outlined"
					/>
					<Button id="search"  onClick={this.handleSubmit} color="primary" autoFocus>
					Search
					</Button>
					</Typography>
				</div>					
			...
			if(!this.state.searchResponse.length){
				...
				return(
					...
					variant="outlined"
					/>
					<Button className="search_button" id="search" onClick={this.handleSubmit} color="primary" autoFocus>
					Search
					</Button>
					</Typography>
	...
	displayGraph(){
		...
		console.log("symbol============>",firstCompanySymbol);
